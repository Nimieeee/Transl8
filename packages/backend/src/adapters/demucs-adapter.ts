...
import { VocalIsolationAdapter, VocalIsolationResult, HealthCheckResult, AdapterMetadata } from './types';
...
export class DemucsAdapter extends VocalIsolationAdapter {
  ...
  async separateVocals(
    ...
    try {
      ...
      if (error.response) {
        throw new Error(
          `Demucs service error: ${error.response.status} - ${error.response.data}`
        );
      } else if (error.code === 'ECONNREFUSED') {
        throw new Error(
          `Cannot connect to Demucs service at ${this.serviceUrl}`
        );
      } else {
