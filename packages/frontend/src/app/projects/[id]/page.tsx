    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 border-b border-[#2a2a38] bg-[#0a0a0f]/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-6 py-6">
          <button
            ...
            className="flex items-center gap-2 text-[#6b6b7f] hover:text-[#ff3366] transition-colors mb-4 group"
          >
            <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" />
            <span className="font-mono text-sm">Back to Studio</span>
          </button>
          <div className="flex items-start justify-between">
            ...
            <div className={`px-4 py-2 rounded-xl text-sm font-mono font-semibold border bg-gradient-to-r ${getStatusColor()} flex items-center gap-2`}>
              {getStatusIcon()}
              {project.status}
            </div>
          </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Upload Section */}
          <div className={`bg-[#13131a] border border-[#2a2a38] rounded-2xl p-8 ${mounted ? 'animate-slide-up opacity-0' : 'opacity-0'}`}>
            <div className="flex items-center gap-3 mb-6">
              <div className="p-3 bg-gradient-to-br from-[#ff3366]/20 to-[#ff3366]/5 rounded-xl border border-[#ff3366]/30">
                <Upload className="w-6 h-6 text-[#ff3366]" />
              </div>
              <div>
                <h2 className="text-xl font-bold text-white">Upload Video</h2>
                <p className="text-sm text-[#6b6b7f] font-mono">MP4, MOV, AVI • Max 500MB</p>
              </div>
            </div>
            ...
            {!project.video_url ? (
              <div className="space-y-4">
                <label className="block">
                  <div className="border-2 border-dashed border-[#2a2a38] rounded-xl p-8 text-center hover:border-[#ff3366] transition-colors cursor-pointer group">
                    <input
                      type="file"
                      accept="video/*"
                      ...
                      className="hidden"
                    />
                    <Film className="w-12 h-12 text-[#6b6b7f] mx-auto mb-3 group-hover:text-[#ff3366] transition-colors" />
                    <p className="text-[#a0a0b8] font-semibold mb-1">
                      {file ? file.name : 'Click to select video'}
                    </p>
                    <p className="text-sm text-[#6b6b7f] font-mono">
                ...
                <button
                  ...
                  className="w-full py-4 bg-gradient-to-r from-[#ff3366] to-[#ff4477] rounded-xl font-bold text-white disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 transition-transform duration-300 flex items-center justify-center gap-2"
                >
                  {uploading ? (
                    <>
