...
- ✅ **Smart LLM Adaptation**: Uses Gemini to adapt translations to target duration
- ✅ **Syllable Estimation**: Calculates target syllable count (3-4 per second)
- ✅ **Fish Audio TTS**: Local TTS with MPS (Apple Silicon) acceleration
...
audio_path, duration, attempts = validator.adapt_and_validate_line(
    ...
    original_audio_path="./reference.wav",
    output_dir="./output"
)
...
- Prompts Gemini to translate with syllable constraint

### 2. Generation (Test)
- Generates audio with Fish Audio
- Uses MPS device for Apple Silicon
- Disables torch.compile for M1 compatibility
...
ATTEMPT 2/3
============================================================
Attempt 2: Requesting LLM adaptation...
LLM returned: Hola, ¿cómo estás tú hoy día?
Generating audio with Fish Audio (MPS)...
Audio generated: ./output/attempt_2_1234567891.wav
...
============================================================
FINAL RESULT:
Audio Path: ./output/attempt_2_1234567891.wav
...
| Feature | Current (OpenAI TTS) | With Validation Loop |
|---------|---------------------|---------------------|
...
### Fish Audio not found
```bash
# Install Fish Audio from source
