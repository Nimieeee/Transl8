...
import { OpenAIWhisperAdapter } from './packages/backend/src/adapters/openai-whisper-adapter';
import { contextMapService } from './packages/backend/src/lib/context-map';
import { AdaptationEngine } from './packages/backend/src/lib/adaptation-engine';
import fs from 'fs';
...
// Initialize Prisma with proper path
...
async function main() {
  ...
  try {
    ...
    execSync(`ffmpeg -i "${videoFile}" -vn -acodec pcm_s16le -ar 16000 -ac 1 "${audioFile}" -y 2>/dev/null`);
    ...
    // Step 4: Create test project in database
    log('\n💾 Step 4: Creating test project...', 'blue');
    
    const project = await prisma.project.create({
      data: {
        name: `CLI Test - ${new Date().toISOString()}`,
        sourceLanguage: 'en',
        targetLanguage: 'es',
        status: 'PROCESSING',
        progress: 0,
    ...
    log(`  Total duration: ${contextMap.totalDuration.toFixed(1)}s`, 'reset');
    ...
    if (!process.env.GEMINI_API_KEY) {
      ...
      log(`  Testing adaptation for: "${testSegment.originalText.substring(0, 50)}..."`, 'reset');
      
      const startAdapt = Date.now();
      const adapted = await adaptationEngine.adaptSegment(
        testSegment.originalText,
        'en',
        'es',
        {
          duration: testSegment.duration,
          emotion: testSegment.emotion,
          previousText: '',
          nextText: contextMap.segments[1]?.originalText || '',
        }
      );
      const adaptTime = Date.now() - startAdapt;
      
      log(`✓ Adaptation complete in ${(adaptTime / 1000).toFixed(1)}s`, 'green');
      log(`  Original: "${testSegment.originalText}"`, 'reset');
      log(`  Adapted: "${adapted.adaptedText}"`, 'reset');
      log(`  Status: ${adapted.status}`, 'reset');
      log(`  Validation: ${adapted.validationPassed ? 'PASSED' : 'FAILED'}`, 'reset');
    }
    ...
    log(`  Processing ratio: ${(transcribeTime / (sttResult.transcript.duration * 1000)).toFixed(2)}x`, 'reset');
    
    log('\n💰 Estimated Cost:', 'blue');
    const minutes = Math.ceil(sttResult.transcript.duration / 60);
    const cost = minutes * 0.006;
    log(`  OpenAI Whisper: $${cost.toFixed(4)} (${minutes} minute${minutes > 1 ? 's' : ''})`, 'reset');
    ...
    log('  1. Start Docker services: docker-compose up -d demucs noisereduce emotion openvoice', 'reset');
    ...
    try {
      await prisma.contextMap.deleteMany({ where: { projectId: project.id } });
